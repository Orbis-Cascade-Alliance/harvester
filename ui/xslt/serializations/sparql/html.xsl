...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:res="http://www.w3.org/2005/sparql-results#"
	xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#" xmlns:harvester="https://github.com/Orbis-Cascade-Alliance/harvester" exclude-result-prefixes="#all" version="2.0">
	...
	<xsl:variable name="namespaces" as="item()*">
		<namespaces>
			...
			<namespace prefix="doap" uri="hhttp://usefulinc.com/ns/doap#"/>
				...
				<xsl:for-each select="*">
					<xsl:choose>
						<xsl:when test="child::*">
							...
							<dd>
								<xsl:apply-templates select="child::node()" mode="suburi"/>
							</dd>							
						</xsl:when>
						<xsl:otherwise>
							<xsl:apply-templates select="self::node()" mode="list-item">
							...
							<span property="{name()}">
								...
								<xsl:choose>
									<xsl:when test="contains(@rdf:datatype, '#gYear')">
										<xsl:value-of select="harvester:normalizeDate(.)"/>
									</xsl:when>
