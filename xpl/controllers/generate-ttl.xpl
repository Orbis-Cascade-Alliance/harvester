...
<p:pipeline xmlns:p="http://www.orbeon.com/oxf/pipeline" xmlns:oxf="http://www.orbeon.com/oxf/processors" xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:xxforms="http://orbeon.org/oxf/xml/xforms">
	...
	<!-- config for file serializer to write TTL to disk -->
	<p:processor name="oxf:unsafe-xslt">		
		<p:input name="data" href="#request"/>
		<p:input name="config">
			<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
				...
				<xsl:template match="/">
					...
					<config>
						<url>
							<xsl:value-of select="concat('oxf:/apps/harvester/download/', $repository, '.ttl')"/>
						</url>
	...
	<!-- execute XSLT transformation from RDF/XML to TTL -->
